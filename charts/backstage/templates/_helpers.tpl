...
{{- if .Values.config.backend.database.connection.ssl.ca -}}
    {{ .Values.config.backend.database.connection.ssl.ca }}
{{- else -}}
/etc/postgresql
{{- end -}}
{{- end -}}
...
{{/*
Generate a password for the postgres user used for the connections from the backend
*/}}
...
{{- else -}}
{{- required "A valid .Values.config.backend.database.connection.host is required when postgresql is not enabled" .Values.config.backend.database.connection.host -}}
{{- end -}}
{{- end -}}
...
{{- else if .Values.config.backend.database.connection.port -}}
{{- .Values.config.backend.database.connection.port }}
{{- else -}}
5432
{{- end -}}
{{- end -}}
...
{{- else -}}
{{- required "A valid .Values.config.backend.database.connection.user is required when postgresql is not enabled" .Values.config.backend.database.connection.user -}}
{{- end -}}
{{- end -}}
...
{{- required "A valid .Values.config.backend.database.connection.password is required when postgresql is not enabled" $secretName -}}
{{- end -}}
{{- end -}}
